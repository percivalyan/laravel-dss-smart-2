    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="header">
                        <h4 class="title">Edit Criteria Non-Academic</h4>
                    </div>
                    <div class="content">
                        <form action="{{ route('criteria.nonacademic.update', $criteriaNonAcademic->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                                ...
                                <select name="criteria_code_id" class="form-control" required>
                                    ...
                                    @foreach ($criteriaCodes as $code)
                                        <option value="{{ $code->id }}"
                                            {{ $criteriaNonAcademic->criteria_code_id == $code->id ? 'selected' : '' }}>
                                            {{ $code->criteria_code }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="criteria_name" class="form-control"
                                    value="{{ $criteriaNonAcademic->criteria_name }}" required>
                            </div>

                            <div class="form-group">
                                <label>Bobot</label>
                                <input type="number" step="any" name="weight" class="form-control"
                                    value="{{ $criteriaNonAcademic->weight }}" required>
                            </div>
